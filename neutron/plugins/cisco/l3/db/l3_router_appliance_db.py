...
from neutron.db.l3_agentschedulers_db import L3_AGENTS_SCHEDULER_OPTS
...
class RouterCreateInternalError(q_exc.NeutronException):
    message = _("Router could not be created due to internal error.")


class RouterInternalError(q_exc.NeutronException):
    message = _("Internal error during router processing.")


class RouterBindingInfoError(q_exc.NeutronException):
...
class L3_router_appliance_db_mixin(extraroute_db.ExtraRoute_db_mixin):
    ...
    # TODO(bobmel): Set to correct value during start of Neutron
    ...
    def _get_profile_id(cls, p_type, resource, name):
        ...
        if p_type == 'net_profile':
            profiles = manager.NeutronManager.get_plugin().get_network_profiles(
                q_context.get_admin_context(),
                {'tenant_id': [tenant_id],
                 'name': [name]},
                ['id'])
        else:
            profiles = manager.NeutronManager.get_plugin().get_policy_profiles(
                q_context.get_admin_context(),
                {'tenant_id': [tenant_id],
                 'name': [name]},
                ['id'])
